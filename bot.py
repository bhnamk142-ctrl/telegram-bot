...
# ==================== دیتابیس SQLITE (ساختار جامع و پیشرفته) ====================
def init_db():
  ...
  # بررسی و اضافه کردن خودکار ستون join_date اگر در دیتابیس‌های قدیمی وجود نداشته باشد
  try:
  ...
  cursor.execute(
      "CREATE TABLE IF NOT EXISTS bot_stats (stat_key TEXT PRIMARY KEY,"
      " stat_value INTEGER)"
  )

  cursor.execute(
      "INSERT OR REPLACE INTO admins VALUES (?)", (MAIN_ADMIN_ID,)
  )
  ...
  default_welcome = (
      ...
      "من اینجام تا کمکت کنم ویدیوها و محتویات دلخواهت رو خیلی سریع و باکیفیت دانلود کنی:\n\n"
      "📥 **اینستاگرام:** دانلود ریلز، پست و استوری با بالاترین کیفیت\n"
      "📺 **یوتیوب:** دانلود ویدیو با رزولوشن‌های مختلف + تبدیل سریع به صوت MP3\n"
      "📱 **تیک‌تاک:** دانلود ویدیوها بدون واترمارک\n"
      "📝 **کپشن‌ساز:** ارسال کامل متن و توضیحات ویدیو همراه با فایل\n\n"
      "⚡ **برای شروع، کافیه لینک ویدیوت رو برام بفرستی تا در چند ثانیه تحویلت بدم!**"
  )
  cursor.execute(
...
# ==================== توابع بررسی ادمین و وضعیت ربات ====================
...
def add_user(user_id):
  ...
  if not exists:
    ...
    cursor.execute(
        "UPDATE users SET usage_count = usage_count + 1, join_date = COALESCE(join_date, ?) WHERE user_id=?",
        (now_str, user_id),
    )
  conn.commit()
  conn.close()
...
# ==================== مدیریت جوین اجباری و پاکسازی خودکار ====================
...
def member_profile_callback(call):
  ...
  text = (
      ...
      f"⏳ وضعیت اشتراک: <b>{vip_status_str}</b>\n"
  ...
  try:
    ...
    try:
      bot.send_message(
          call.message.chat.id, text, reply_markup=kb, parse_mode="HTML"
      )
    except Exception:
      pass
...
def member_help_callback(call):
  ...
  text = (
      ...
      "1️⃣ کافیست لینک ویدیو مورد نظر خود را از پلتفرم‌های **اینستاگرام، یوتیوب"
      " یا تیک‌تاک** کپی کنید.\n"
      "2️⃣ لینک را مستقیماً در همین چت ارسال نمایید.\n"
      "3️⃣ کیفیت دلخواه خود (یا صوت MP3) را انتخاب کنید.\n"
      "4️⃣ ربات ویدیو را به همراه کپشن کامل برای شما دانلود کرده و ارسال می‌کند."
  )
  kb = types.InlineKeyboardMarkup()
  kb.add(
      types.InlineKeyboardButton(
...
def buy_vip_stars_handler(call):
  ...
  try:
    ...
    bot.send_message(
        call.message.chat.id, f"❌ خطا در ایجاد فاکتور پرداخت: {e}"
    )
...
def show_admin_panel_content(chat_id, message_id=None):
  ...
  text = (
      ...
      f"🎨 رنگ تم دکمه‌ها: <b>{get_setting('theme_color').upper()}</b>\n\n"
...
def process_manual_vip(message):
  ...
  try:
    ...
    msg = bot.send_message(
        message.chat.id, "❌ لطفاً فقط یک عدد صحیح (آیدی عددی) بفرستید:"
    )
    bot.register_next_step_handler(msg, process_manual_vip)
...
def admin_section_settings(call):
  ...
  try:
    ...
    pass
  text = "⚙️ <b>تنظیمات عمومی ربات</b>\n\nنکته: برای نام کاربر در متن خوش‌آمدگویی از <code>{first_name}</code> استفاده کنید.\nاز دکمه‌های زیر برای تغییرات استفاده کنید:"
  kb = types.InlineKeyboardMarkup(row_width=1)
  kb.add(
      types.InlineKeyboardButton(
...
def handle_admin_callbacks(call):
  ...
  if data == "adm_clear_fj":
    ...
    msg = bot.send_message(
        call.message.chat.id,
        "💬 <b>متن خوش‌آمدگویی جدید را ارسال کنید:</b>\n(توجه: برای نمایش خودکار نام کاربر، از <code>{first_name}</code> در متن خود استفاده کنید)",
    )
...
# ==================== دریافت لینک و دانلود با سیستم ضد فریز ====================
